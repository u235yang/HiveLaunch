...
use crate::executors::{copilot::Copilot, gemini::Gemini, qwen::QwenCode};
use crate::{
...
pub mod amp;
pub mod claude;
pub mod codex_stub; // stub for LogWriter (Codex executor disabled)
pub mod copilot;
pub mod cursor;
pub mod droid;
pub mod gemini;
...
impl CodingAgent {
    ...
    pub fn capabilities(&self) -> Vec<BaseAgentCapability> {
        match self {
            ...
            Self::Gemini(_) | Self::QwenCode(_) => vec![BaseAgentCapability::SessionFork],
...
pub trait StandardCodingAgentExecutor {
    ...
    /// Returns a stream of executor discovered options updates (unified API).
    /// This includes slash commands, models, and agents.
    async fn discover_options(
        &self,
        _workdir: Option<&Path>,
        _repo_path: Option<&Path>,
    ) -> Result<BoxStream<'static, json_patch::Patch>, ExecutorError> {
        // Default implementation: return empty options
        use crate::model_selector::ExecutorDiscoveredOptions;
        Ok(Box::pin(futures::stream::once(async move {
            patch::executor_discovered_options(ExecutorDiscoveredOptions::default())
        })))
    }
    ...
    async fn spawn_review(
        ...
        match session_id {
            Some(id) => {
                self.spawn_follow_up(current_dir, prompt, id, None, env, None, None)
                    .await
            }
            None => self.spawn(current_dir, prompt, env, None).await,
        }
    }
