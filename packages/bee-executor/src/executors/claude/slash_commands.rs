...
impl ClaudeCode {
    ...
    /// Discover available slash commands (legacy method, caching is now handled by discover_options)
    /// Note: This method no longer uses SlashCommandCache - caching is now unified via ExecutorOptionsCache
    pub async fn discover_available_slash_commands(
        &self,
        current_dir: &Path,
    ) -> Result<Vec<SlashCommandDescription>, ExecutorError> {
        // Note: We don't use SlashCommandCache here anymore - caching is now handled by 
