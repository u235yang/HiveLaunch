...
use bee_workspace_utils::{approvals::ApprovalStatus, msg_store::MsgStore, path::make_path_relative};
...
pub fn normalize_logs(msg_store: Arc<MsgStore>, worktree_path: &Path) {
    ...
    tokio::spawn(async move {
        ...
        while let Some(Ok(line)) = stdout_lines.next().await {
            line_count += 1;
            // 🔍 日志：记录收到的每一行 stdout（截取前 500 字符避免日志过长）
            let preview: String = line.chars().take(500).collect();
            tracing::info!(
                "[NORMALIZE_LOGS] Line #{}: {}",
                line_count,
                if line.len() > 500 { format!("{}... (truncated, total {} bytes)", preview, line.len()) } else { preview.clone() }
            );
            ...
            let Some(event) = parse_event(&line) else {
                ...
                tracing::warn!("[NORMALIZE_LOGS] Failed to parse as OpencodeExecutorEvent: {}", trimmed);
            ...
            // 🔍 日志：记录解析后的事件类型
            tracing::info!("[NORMALIZE_LOGS] Parsed event: {:?}", std::mem::discriminant(&event));
            ...
            match event {
                ...
                OpencodeExecutorEvent::SdkEvent { event } => {
                    // 🔍 日志：记录 SDK 事件
                    tracing::info!("[NORMALIZE_LOGS] SdkEvent received, raw: {}", serde_json::to_string(&event).unwrap_or_default());
        ...
        match part {
            Part::Text(part) => {
                if self.message_roles.get(&part.message_id) == Some(&MessageRole::User) {
                    tracing::debug!(
                        "Skipping text part for user message_id {}",
                        part.message_id
                    );
                    return;
                }
            ...
            ToolData::FileEdit {
                ...
                let changes = match kind {
                    ...
                    FileEditKind::Edit | FileEditKind::MultiEdit => unified_diff
                        ...
                        .map(|d| {
                            vec![FileChange::Edit {
                                unified_diff: bee_workspace_utils::diff::normalize_unified_diff(
                                    &path, d,
                                ),
