...
use crate::{
    ...
    executors::{
        ...
        opencode::Opencode,
        utils::{SlashCommandCall, parse_slash_command, reorder_slash_commands},
    },
};
...
impl Opencode {
    /// Discover slash commands (legacy method, use discover_all instead for better performance)
    /// Note: This method no longer uses SlashCommandCache - caching is now handled by discover_options
    pub async fn discover_slash_commands(
        &self,
        current_dir: &Path,
    ) -> Result<Vec<SlashCommandDescription>, ExecutorError> {
        // Note: We don't use SlashCommandCache here anymore - caching is handled by 
        // discover_options which uses the unified ExecutorOptionsCache
