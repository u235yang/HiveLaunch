...
use bee_workspace_utils::approvals::ApprovalStatus;
...
impl LogWriter {
    ...
    pub async fn log_event(&self, event: &OpencodeExecutorEvent) -> Result<(), ExecutorError> {
        ...
        // 🔍 日志：记录 LogWriter 写入的事件
        tracing::info!("[LOG_WRITER] Writing event: {} bytes", raw.len());
...
pub async fn wait_for_health(
    ...
    tracing::info!("[wait_for_health] Starting health check for: {}", health_url);

    loop {
        if tokio::time::Instant::now() > deadline {
            tracing::error!("[wait_for_health] Timed out after 20s, last error: {:?}", last_err);
        ...
        match resp {
            Ok(resp) => {
                tracing::info!("[wait_for_health] Got response: status={}", resp.status());
                if !resp.status().is_success() {
                    last_err = Some(format!("HTTP {}", resp.status()));
                    // 如果是 401/403，打印更多诊断信息
                    if resp.status().as_u16() == 401 || resp.status().as_u16() == 403 {
                        tracing::error!("[wait_for_health] Auth failed! Status: {}, headers: {:?}", 
                            resp.status(), resp.headers());
                    }
                } else if let Ok(body) = resp.json::<HealthResponse>().await {
                    if body.healthy {
                        tracing::info!("[wait_for_health] Server is healthy!");
                        return Ok(());
                    }
            ...
            Err(err) => {
                tracing::warn!("[wait_for_health] Request failed: {}", err);
...
pub async fn create_session(
    ...
    tracing::info!("[create_session] POST {} with directory={}", url, directory);
    ...
    tracing::info!("[create_session] Response status: {}", status);
    
    if !status.is_success() {
        let error_body = resp.text().await.unwrap_or_default();
        tracing::error!("[create_session] Failed: HTTP {} - {}", status, error_body);
        return Err(ExecutorError::Io(io::Error::other(format!(
    ...
    let session = resp
        ...
        .await
        .map_err(|err| ExecutorError::Io(io::Error::other(err)))?;
    tracing::info!("[create_session] Session created: {}", session.id);
    Ok(session.id)
}
...
pub async fn connect_event_stream(
    ...
    tracing::info!("[connect_event_stream] GET {} with directory={}", url, directory);
    ...
    let resp = req
        ...
        .await
        .map_err(|err| {
            tracing::error!("[connect_event_stream] Request failed: {}", err);
            ExecutorError::Io(io::Error::other(err))
        })?;

    // 检查状态但不消耗 response
    let status = resp.status();
    tracing::info!("[connect_event_stream] Response status: {}", status);
    
    if !status.is_success() {
        // 把错误信息包含在返回的错误中
        return Err(ExecutorError::Io(io::Error::other(format!(
            "OpenCode event stream failed: HTTP {status}"
        ))));
    }

    tracing::info!("[connect_event_stream] SSE connection established successfully!");
    Ok(resp)
}
...
async fn process_event_stream(
    ...
    loop {
        ...
        // 🔍 日志：记录收到的 SSE 事件数据
        tracing::info!("[SSE_EVENT] Received event data ({} bytes): {}", trimmed.len(), trimmed.chars().take(300).collect::<String>());

        let Ok(data) = serde_json::from_str::<Value>(trimmed) else {
            tracing::error!("[SSE_EVENT] Failed to parse as JSON: {}", trimmed);
            let _ = ctx
                .log_writer
                .log_error(format!(
        ...
        let Some(event_type) = data.get("type").and_then(Value::as_str) else {
            tracing::warn!("[SSE_EVENT] Event has no 'type' field: {}", trimmed);
            continue;
        };

        // 🔍 日志：记录事件类型
        tracing::info!("[SSE_EVENT] Event type: {}", event_type);

        if !event_matches_session(event_type, &data, ctx.session_id) {
            tracing::debug!("[SSE_EVENT] Event doesn't match session {}, skipping", ctx.session_id);
            continue;
        }
        ...
        match event_type {
            ...
            "session.status" => {
                // Check if the status type is "idle"
                if let Some(status) = data
                    .pointer("/properties/status/type")
                    .and_then(Value::as_str)
                {
                    tracing::info!("[SSE_EVENT] session.status type: {}", status);
...
fn event_matches_session(event_type: &str, event: &Value, session_id: &str) -> bool {
    let extracted = match event_type {
        "message.updated" => event
            ...
            .and_then(Value::as_str),
        "message.part.updated" | "message.part.delta" => event
            .pointer("/properties/part/sessionID")
            .and_then(Value::as_str),
        "permission.asked" | "permission.replied" | "session.idle" | "session.status" | "session.error" => event
