...
use crate::{
    ...
    executors::{
        AppendPrompt, AvailabilityInfo, BaseCodingAgent, ExecutorError, SpawnedChild, StandardCodingAgentExecutor,
        codex_stub::LogWriter, utils::{executor_options_cache, reorder_slash_commands},
    },
    logs::{
    ...
    model_selector::{claude_default_model_selector, ExecutorDiscoveredOptions},
...
impl StandardCodingAgentExecutor for ClaudeCode {
    ...
    async fn discover_options(
        ...
        if let Some(wd) = &current_dir {
            let target_key = ExecutorConfigCacheKey::new(Some(wd), String::new(), base_executor);
            if let Some(cached) = cache.get(&target_key) {
                tracing::info!("[discover_options] Cache hit for Claude Code dir: {}", wd.display());
                return Ok(Box::pin(futures::stream::once(async move {
                    patch::executor_discovered_options(cached.as_ref().clone().with_loading(false))
                })));
            }
            // Also try global cache as fallback
            let global_key = ExecutorConfigCacheKey::new(None, String::new(), base_executor);
            if let Some(cached) = cache.get(&global_key) {
                tracing::info!("[discover_options] Global cache hit for Claude Code");
                return Ok(Box::pin(futures::stream::once(async move {
                    patch::executor_discovered_options(cached.as_ref().clone().with_loading(false))
                })));
            }
        ...
        // Cache miss - return loading state, then discover
        // Initial state: loading, with default model selector
        let initial_options = ExecutorDiscoveredOptions {
            model_selector: claude_default_model_selector(),
        ...
        let discovery_stream = futures::stream::once(async move {
            match current_dir {
                Some(dir) => match this.discover_available_slash_commands(&dir).await {
                    Ok(commands) => {
                        ...
                        // Store in cache (both target and global)
                        let cache = executor_options_cache();
                        let dir_ref = &dir;
                        let target_key = ExecutorConfigCacheKey::new(Some(dir_ref), String::new(), base_executor);
                        cache.put(target_key, final_options.clone());
                        tracing::info!("[discover_options] Cached options for Claude Code dir: {}", dir.display());
                        
                        // Also cache globally
                        let global_key = ExecutorConfigCacheKey::new(None, String::new(), base_executor);
                        cache.put(global_key, final_options.clone());
                        tracing::info!("[discover_options] Cached global options for Claude Code");

                        patch::executor_discovered_options(final_options)
                    }
                    Err(e) => {
                        tracing::warn!("Failed to discover Claude Code slash commands: {}", e);
                        let error_options = ExecutorDiscoveredOptions {
                            model_selector: claude_default_model_selector(),
                            slash_commands: defaults,
                            loading_models: false,
                            loading_agents: false,
                            loading_slash_commands: false,
                            error: Some(e.to_string()),
                        };
                        patch::executor_discovered_options(error_options)
                    }
                },
                None => {
                    let final_options = ExecutorDiscoveredOptions {
    ...
    fn normalize_entries(
        ...
        match claude_json {
            ...
            ClaudeJson::ApprovalResponse {
                ...
                // Convert denials and timeouts to visible entries
...
mod tests {
    ...
    async fn test_streaming_patch_generation() {
        ...
        let patch_count = history
            .iter()
            .filter(|msg| matches!(msg, bee_workspace_utils::log_msg::LogMsg::JsonPatch(_)))
            .count();
        assert!(
            patch_count > 0,
