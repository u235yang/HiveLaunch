...
use thiserror::Error;
use ts_rs::TS;
use bee_workspace_utils::shell::resolve_executable_path;
...
impl CommandParts {
    ...
    pub async fn into_resolved(self) -> Result<(PathBuf, Vec<String>), ExecutorError> {
        ...
        tracing::info!("[into_resolved] Looking for executable: {}, PATH={}", 
            program, std::env::var_os("PATH").map(|p| p.to_string_lossy().to_string()).unwrap_or_default());
        let executable = resolve_executable_path(&program)
            ...
            .ok_or_else(|| {
                ...
                let path = std::env::var_os("PATH")
                    ...
                    .unwrap_or_default();
                tracing::error!("[into_resolved] Executable '{}' not found in PATH. PATH={}", program, path);
                ExecutorError::ExecutableNotFound { program }
            })?;
        tracing::info!("[into_resolved] Found executable: {:?}", executable);
        Ok((executable, args))
    }
}
