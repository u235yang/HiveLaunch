...
impl ExecutorConfigs {
    ...
    pub fn get_cached() -> ExecutorConfigs {
        match EXECUTOR_PROFILES_CACHE.read() {
            Ok(guard) => guard.clone(),
            Err(poisoned) => {
                tracing::error!(
                    "Executor profile cache lock poisoned while reading; recovering with in-memory value"
                );
    ...
    pub fn reload() {
        match EXECUTOR_PROFILES_CACHE.write() {
            Ok(mut cache) => {
                *cache = Self::load();
            }
            Err(poisoned) => {
                tracing::error!(
                    "Executor profile cache lock poisoned while reloading; recovering lock state"
                );
    ...
    pub fn load() -> Self {
        let profiles_path = bee_workspace_utils::assets::profiles_path();
