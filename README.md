...
**适合**：想在本地掌控 AI 执行流程的开发者和团队
**不适合**：需要云端 AI SaaS 的用户
...
| 功能 | 说明 |
|---|---|
| 场景脚手架 | 从模板快速生成项目骨架与 AI 执行规则 |
| 看板任务执行 | 把需求拆成卡片并派发给 AI 执行，全流程可追踪 |
| 会话与执行面板 | 同一界面查看对话、命令输出、文件改动与执行状态 |
| Swarm 配置中心 | 统一配置 Agent、Skills、MCP，按项目范围生效 |
| Worktree 隔离执行 | 每个任务在独立分支和目录执行，降低污染风险 |
| 本地优先数据控制 | 代码与数据默认留在你的设备 |
