...
use crate::workspace_utils::{approvals::ApprovalStatus, msg_store::MsgStore, path::make_path_relative};
...
pub fn normalize_logs(msg_store: Arc<MsgStore>, worktree_path: &Path) {
    ...
    tokio::spawn(async move {
        log::info!("[normalize_logs] Started for worktree {:?}", worktree_path);
        let mut stored_session_id = false;
        let mut state = LogState::new(entry_index.clone(), msg_store.clone());

        let mut stdout_lines = msg_store.stdout_lines_stream();
        let mut line_count = 0usize;
        log::info!("[normalize_logs] Starting to read stdout_lines_stream");
        while let Some(Ok(line)) = stdout_lines.next().await {
            line_count += 1;
            log::info!(
                "[normalize_logs] Received stdout line #{}: {} chars",
                line_count,
                line.len()
            );
            let Some(event) = parse_event(&line) else {
                ...
                log::info!("[normalize_logs] Parsed as system message: {}", trimmed.chars().take(100).collect::<String>());
            ...
            log::info!("[normalize_logs] Parsed event: {:?}", std::mem::discriminant(&event));
            ...
            ToolData::FileEdit {
                ...
                let changes = match kind {
                    ...
                    FileEditKind::Edit | FileEditKind::MultiEdit => unified_diff
                        ...
                        .map(|d| {
                            vec![FileChange::Edit {
                                unified_diff: crate::workspace_utils::diff::normalize_unified_diff(
                                    &path, d,
                                ),
