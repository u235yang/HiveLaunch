...
use crate::workspace_utils::{diff::Diff, msg_store::MsgStore};
...
pub fn upsert_normalized_entry(
    ...
    log::info!(
        "[patch] upsert_normalized_entry: index={}, is_new={}, content_preview={}",
        index,
        is_new,
        normalized_entry.content.chars().take(50).collect::<String>()
    );
    if is_new {
...
pub fn add_normalized_entry(
    ...
    log::info!(
        "[patch] add_normalized_entry: index={}, entry_type={:?}",
        index,
        std::mem::discriminant(&normalized_entry.entry_type)
    );
